...
const HeartIcon: React.FC<Props> = ({ isFavorite }) => {
  // Svg when heart is fully red
  const likedHeart: string =
  ...
  // Svg when heart icon is only outlined
  const NoLikedHeart: string =
  ...
  return (
    ...
    >
      <path fill="#DD0031" d={isFavorite ? likedHeart : NoLikedHeart}></path>
    </svg>
  );
};
